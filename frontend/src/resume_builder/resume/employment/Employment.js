...
function Company({ order }) {
    ...
    <li className="">
        <div className="row">
            <div className="col-3">
                <input name={`company-period-${order}`} className='left-max' placeholder={`working period ${order}`} value={state["company-period"]} onChange={(e) => setState(e)} />
            </div>
            <div className="col-6">
                <input name={`company-name-${order}`} className='center-max' placeholder={`company's name ${order}`} value={state["company-name"]} onChange={(e) => setState(e)} />
            </div>
            <div className="col-3">
                <input name={`company-location-${order}`} className='right-max' placeholder={`location ${order}`} value={state["company-location"]} onChange={(e) => setState(e)} />
            </div>
        </div>
        <div className="details">
            <textarea name={`company-detail-${order}`} placeholder={`more details ${order}`} value={state["company-detail"]} onInput={(e) => setState(e)} />
        </div>
    </li>
    )
};
